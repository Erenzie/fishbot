	...
	if($fb->rawcmd == "PRIVMSG") {
		...
		switch($fb->cmd) {
			...
			case "addregexreply":
				...
				break;
			case "myperms":
				if($fb->hasPerm("admin", 0)) { $fb->sndMsg($fb->chan, "You have admin"); } else { $fb->sndMsg($fb->chan, "You don't have admin"); }
				if($fb->hasPerm("addregexes", 0)) { $fb->sndMsg($fb->chan, "You have addregexes"); } else { $fb->sndMsg($fb->chan, "You don't have addregexes"); }
				if($fb->hasPerm("delregexes", 0)) { $fb->sndMsg($fb->chan, "You have delregexes"); } else { $fb->sndMsg($fb->chan, "You don't have delregexes"); }
				if($fb->hasPerm("delquotes", 0)) { $fb->sndMsg($fb->chan, "You have delquotes"); } else { $fb->sndMsg($fb->chan, "You don't have delquotes"); }
				break;
			case "8ball":
				$responses = array(
					"Forget about it.",
			...
			default:
				if($fb->somethingmatched == 0) { // this doesn't even work, why do I have this here? :|
					...
					/*if(preg_match("/\001ACTION (.*) {$fb->botnick}( )?\001$/i", $fb->msg, $result)) {
					...
					} else*/ if($fb->msg == "what's in the cup?") {
						if(rand(0, 1) == 0) {
						...
						if(substr($blah[2], -1) == "!") { $blah[2] = substr($blah[2], 0, -1); }
