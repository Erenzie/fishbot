...
class bdBattle {
	...
	function doRespawn($player, $botobj) {
		...
		if($numdeaths == 1 or substr($numdeaths, -1) == "1" && $numdeaths !== 11) {
			$a = "{$numdeaths}st";
		} elseif($numdeath == 2 or substr($numdeaths, -1) == "2" && $numdeaths !== 12) {
			$a = "{$numdeaths}nd";
	...
	function doAttacking($attacker, $victim, $weapon, $nofail=false) {
		...
		if($decideIfCrit > 90) { $isCrit = true; } else { $isCrit = false; }
		...
		if(!$nofail) {
			...
			if($decideIfMiss > 90) { $isMiss = true; $damagetodeal = 0; } else { $isMiss = false; }
		} else {
			$isMiss = false;
		}
	...
	function checkAttkMatch($regex, $string, $botobj, $altorder = false) {
		...
		if(preg_match("/^\001ACTION {$regex}\001$/i", $string, $blah)) {
			...
			if($altorder) { $victim = $blah[2]; } else { $victim = $blah[1]; }
			$this->victim = trim($victim);
			if($this->checkIsVictimSelf($victim)) { $this->victim = $botobj->nick; }
			if($altorder) { $this->weapon = $blah[1]; } else { $this->weapon = $blah[2]; }
	...
	function checkForHealCmd($string, $botobj) {
		if(preg_match("/^\001ACTION heals (.*) with (.*)\001$/i", $string, $blah)) {
			...
			if($this->checkIsVictimSelf($this->patient)) { $this->patient = $botobj->nick; }
			$this->tool = $blah[2];
			return true;
		} else {
			$this->healer = null;
	...
	function doHealing($patient, $healer, $tool) {
		...
		if($decideIfSuccess < 50) { $isSuccess = true; } else { $isSuccess = false; }
